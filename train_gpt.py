...
class Muon(optim.Optimizer):
    """
    Momentum + Newton-Schulz, with one reduce_scatter per bucket
    and ordered broadcasts to keep replicas in sync.
    """
    ...
    def __init__(
        ...
        *,
        ...
        for size in {p.numel() for p in params}:
            # distinct tensor for each row → no aliasing
            buf_list = [
                torch.empty(size, dtype=torch.bfloat16, device="cuda")
                for _ in range(world_size)
            ]
            pg.append(
                dict(
                    params=[p for p in params if p.numel() == size],
                    buf_list=buf_list,          # list[Tensor]
                    row=buf_list[rank],          # this rank's send/recv buf
                )
            )
        super().__init__(pg, defaults)

    # ---------------------------------------------------------------------
    @torch.no_grad()
    def step(self, closure=None):
        ws, rk = self.ws, self.rank
        for g in self.param_groups:
            params: list[Tensor] = g["params"]
            buf_list: list[Tensor] = g["buf_list"]
            row: Tensor = g["row"]
            lr, wd = g["lr"], g["weight_decay"]
            mom, nest, ns_steps = g["momentum"], g["nesterov"], g["ns_steps"]

            handle = None
            base_prev = None  # start index of the in-flight bucket

            def finish_prev():
                handle.wait()
                # owner rank = offset inside the bucket
                owner = (base_prev + rk) % ws
                # param index this rank owns in that bucket
                p_idx = base_prev + rk
                if p_idx < len(params):          # valid param
                    p = params[p_idx]
                    p.mul_(1 - lr * wd * getattr(p, "wd_mul", 1.0))
                    p.add_(
                        row.view_as(p),
                        alpha=-lr * max(1, p.size(-2) / p.size(-1)) ** 0.5,
                    )
                # ordered broadcasts (roots 0 … ws-1)
                for offset in range(ws):
                    idx = base_prev + offset
                    if idx >= len(params):
                        continue
                    root = offset
                    dist.broadcast(params[idx], src=root)

            # iterate buckets as in original script
            for base in range(0, len(params), ws):
                if handle is not None:
                    finish_prev()

                base_prev = base

                # zero previous contents
                for t in buf_list:
                    t.zero_()

                if base + rk < len(params):
                    p = params[base + rk]
                    grad = p.grad
                    ...
                    st["mom"].lerp_(grad, 1 - mom)
                    m = grad.lerp(st["mom"], mom) if nest else st["mom"]

                    # Newton-Schulz
                    upd = zeropower_via_newtonschulz5(m, ns_steps).flatten()
                    row.copy_(upd)

                # async reduce_scatter (SUM)
                handle = dist.reduce_scatter(
                    row, buf_list, op=dist.ReduceOp.SUM, async_op=True
                )
