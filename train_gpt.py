...
# Split into square and rectangular matrices
square_matrix_params = [p for p in hidden_matrix_params if p.size(0) == p.size(1)]
rectangular_matrix_params = [p for p in hidden_matrix_params if p.size(0) != p.size(1)]
...
# Two separate Muon optimizers: one for square matrices, one for rectangular matrices (with half the learning rate)
optimizer2 = Muon(square_matrix_params, lr=0.025, momentum=0.95, rank=rank, world_size=world_size)
optimizer3 = Muon(rectangular_matrix_params, lr=0.05, momentum=0.95, rank=rank, world_size=world_size)
